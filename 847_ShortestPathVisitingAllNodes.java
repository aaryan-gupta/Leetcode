		...
		while (!qu.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int neighbor : graph[nodeId]) {
					...
					// If the same node was visited again with same visitedNodeBit, it means this
					// node can be skipped, For example: 1->0->1->0. First 1 we have {1, 10}, then
					// we have {0, 11}, then we will have {1, 11}. Lastly, we have {0, 11} which is
					// a state we already had before. So we don't visit this again.
